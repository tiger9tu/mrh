gpu_run=1 #toggle for on-off gpu usage
import numpy as np
if gpu_run: from mrh.my_pyscf.gpu import libgpu
import pyscf
if gpu_run: from gpu4mrh import patch_pyscf 
from pyscf import gto, scf, mcscf, lib
from pyscf.csf_fci import csf_solver
from mrh.my_pyscf.dmet import runDMET
if gpu_run: gpu=libgpu.init()
if gpu_run: libgpu.set_verbose_(gpu,1) #optional for output abut performance and gpu usage
lib.logger.TIMER_LEVEL=lib.logger.INFO

'''
Note: In this example, I am only showing CASCI, CASSCF and SA-CASSCF calculations in embedded DMET environment.
The FCIsolvers, conv_tol, level_shifts and other parameters can be changed as per the requirement.
'''

np.set_printoptions(precision=4)

if gpu_run: mol = gto.Mole(basis='6-31G', spin=1, charge=0, verbose=4, max_memory=100000, use_gpu = gpu)
else: mol = gto.Mole(basis='6-31G', spin=1, charge=0, verbose=4, max_memory=100000)
mol.atom='''
P  -5.64983   3.02383   0.00000
H  -4.46871   3.02383   0.00000
H  -6.24038   2.19489   0.59928
Ne 0 0 10
'''
if gpu_run: mol.output = 'gpu_test.log'
else: mol.output = 'cpu_test.log'
mol.build()

mf = scf.ROHF(mol).density_fit()
mf.kernel()

dmet_mf, mydmet = runDMET(mf, lo_method='lowdin', bath_tol=1e-10, atmlst=[0, ])
if gpu_run: dmet_mf.mol.use_gpu = gpu #only change needed to run DMET high level solver (only CAS for now) on GPUs

# Sanity Check
assert abs((mf.e_tot - dmet_mf.e_tot)) < 1e-7, "Something went wrong."

# Active space guess
ao2eo = mydmet.ao2eo
ao2co = mydmet.ao2co
mo_coeff = ao2eo @ dmet_mf.mo_coeff

# Based on the AO Character
from pyscf.tools import mo_mapping
orblst = mo_mapping.mo_comps(['P 3s', 'P 3p', 'H 1s'], mol, mo_coeff, orth_method='meta-lowdin')
orblst = orblst.argsort()

# CASCI Calculation
mc = mcscf.CASCI(dmet_mf, 6, 7)
mo = mc.sort_mo(orblst[-mc.ncas:], base=0)
mc.fcisolver  = csf_solver(mol, smult=2)
mc.kernel(mo)

# CASSCF Calculation
mc = mcscf.CASSCF(dmet_mf, 6, 7)
mo = mc.sort_mo(orblst[-mc.ncas:], base=0)
mc.fcisolver  = csf_solver(mol, smult=2)
mc.kernel(mo)

# SA-CASSCF Calculation
mc = mcscf.CASSCF(dmet_mf, 6, 7)
mo = mc.sort_mo(orblst[-mc.ncas:], base=0)
mc.fcisolver  = csf_solver(mol, smult=2)
mc = mcscf.state_average_(mc, weights=[0.5, 0.5])
mc.kernel(mo)
if gpu_run: libgpu.destroy_device(gpu) #always good to clean up after yourself.
#also gives you gpu usage statistics if you turn on the verbosity. 
